    ...
    const PartySync = (function() {
      ...
      function _notify() {
        _callbacks.forEach(function(fn) {
          try { fn(_partyMap); } catch(e) {
            if (window.TS && TS.debug) TS.debug.log('[PartySync] callback error: ' + e);
          }
        });
      }
      ...
      function broadcastPortraitAndInfo() {
        ...
        img.onerror = function() {
          if (window.TS && TS.debug) TS.debug.log('[PartySync] portrait load failed, chunks not sent');
        };
      ...
      function handleIncoming(senderId, event) {
        ...
        if (msg.t === 'ci') {
          ...
          var assembled = reassembleChunks(buf.chunks, buf.total);
          if (assembled !== null) {
            if (!_partyMap[senderId]) _partyMap[senderId] = _skeleton(senderId);
            _partyMap[senderId].portraitUrl   = 'data:image/jpeg;base64,' + assembled;
    ...
    function renderPartyPanel(party) {
      ...
      var pane   = document.getElementById('party-pane');
      ...
      if (window.TS && window.TS.dice && typeof window.TS.dice.putDiceInTray === 'function') {
        try {
          if (mode === 'normal') {
            ...
            if (window.TS && TS.debug) TS.debug.log('[rollCheck] adv/dis calling putDiceInTray');
